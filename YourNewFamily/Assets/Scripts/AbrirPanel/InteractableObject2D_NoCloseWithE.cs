...
public class InteractableObject2D_NoCloseWithE : MonoBehaviour
{
    ...
    {
        // SOLO abrir el panel con E, nunca cerrarlo con esta tecla
        if (estaMirando && Input.GetKeyDown(teclaInteraccion) && !interfazAbierta)
        {
            AbrirInterfaz();
        }
    }
    ...
    public void AbrirInterfaz()
    {
        ...
        Debug.Log("Panel ABIERTO - Solo se puede cerrar con Escape o alejándose");
    }
    ...
    {
        // Modificado para solo abrir si está cerrado (no toggle completo)
        if (!interfazAbierta)
        {
            AbrirInterfaz();
        }
